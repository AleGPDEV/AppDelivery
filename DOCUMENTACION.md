...
- **Login de admin** (`admin_users` en Supabase, un solo usuario): `POST /api/login` verifica usuario/contraseña (`bcryptjs`) y devuelve una cookie httpOnly firmada con JWT (`SESSION_SECRET`, 30 días). `POST /api/logout` la borra. `POST /api/change-password` permite cambiarla estando logueado. Si la tabla `admin_users` está vacía al arrancar, `bootstrapAdmin()` crea el usuario inicial con `ADMIN_USERNAME`/`ADMIN_PASSWORD` (variables de entorno, solo se usan esa primera vez).
  - **Gate de páginas**: un middleware de Express redirige a `/login.html` si falta la cookie válida al pedir `pedidos.html`, `dashboard.html`, `caja.html`, `analiticas.html`, `catalogo.html` o `proveedores.html`. Esas 6 rutas, si la cookie es válida, caen en `app.get(PROTECTED_PAGES, ...)` (justo antes de `express.static`) que sirve siempre `public/admin-shell.html` — ver "Las páginas de admin son una SPA" más abajo. `driver.html`, `login.html` y `pedido-cliente.html` quedan públicas y siguen siendo archivos HTML propios de siempre — cualquier archivo en `public/` que no esté en `PROTECTED_PAGES` se sirve tal cual, no hace falta nada especial para publicar una página nueva.
...
- `public/geo.js` — misma lógica de geocodificación/ruteo/expansión de links que `optimizador-rutas/app.js`, compartida por las vistas/páginas que la necesitan (las vistas Nuevo pedido/Pedidos de la SPA, `driver.html` — Dashboard y Rendición de caja no la usan). También parsea el formato extendido de carga (ver 3.1) y `parseAmount` (convierte "$ 1.630,00" a número). Script clásico, no módulo — expone `window.Geo`.
...
`pedidos.html`, `dashboard.html`, `caja.html`, `analiticas.html`, `catalogo.html` y `proveedores.html` dejaron de ser documentos HTML independientes — las 6 URLs siguen existiendo tal cual (bookmarks, recarga dura, el gate de auth de arriba no cambia), pero ahora todas sirven el mismo `public/admin-shell.html`, y cambiar de pestaña es una navegación del lado del cliente (sin recargar el documento, sin abrir una conexión de socket nueva). `login.html`, `driver.html` y `pedido-cliente.html` **no** son parte de esto — siguen siendo páginas MPA de siempre, audiencias distintas (público general/delivery) que no se benefician de compartir un socket con el panel de admin. (Hubo una séptima vista, "Nuevo pedido" — se sacó como pestaña propia y ahora es un modal dentro de Pedidos, ver más abajo.)
...
- **`public/js/views/*.js`** (`nuevo-pedido.js`, `pedidos.js`, `dashboard.js`, `caja.js`, `analiticas.js`, `catalogo.js`, `proveedores.js`) — un `<script type="module">` por vista (scope real por archivo, sin colisiones aunque los 7 coexistan en el mismo documento — antes, como `<script>` clásicos, 6 de las 7 declaraban `const socket`/`let formConfig` a nivel de módulo, que hoy chocarían si convivieran). Cada uno registra su vista con `Router.register(path, {...})`. Todo `document.getElementById`/`addEventListener` que antes vivía a nivel de módulo (funcionaba porque el `<script>` estaba al final del `<body>`, después de que el HTML ya existía) se movió adentro de `mount(root)`, consultando `root.querySelector(...)` — la vista se monta recién cuando su HTML ya está en el DOM. `unmount()` reemplaza el contenedor, así que los listeners atados a esos nodos se recolectan solos; solo hace falta limpiar a mano lo que vive *fuera* del DOM: suscripciones a `Store`, timers (`analiticas.js` tiene un debounce de 600ms para el efectivo inicial), y en `dashboard.js` el `setInterval` de 5s + los markers/InfoWindows de Google Maps (`marker.setMap(null)`).
  - `public/js/route-helper.js` (`recomputeRouteForDriver`) y `public/js/driver-label.js` (`createDriverLabel`, el `driverLabel(id)` con fallback "delivery desconectado") — antes vivían duplicados byte a byte en `nuevo-pedido.js`/`pedidos.js` y en `pedidos.js`/`caja.js` respectivamente ("kept independent since these are separate pages", decía el comentario viejo) — ahora que las vistas comparten documento, un solo helper alcanza para cada uno.
  - **`dashboard.js`** es la vista más particular: `loadGoogleMaps()`/su promesa de caché quedan a nivel de módulo (no adentro de `mount()`) para reusar la carga del script de Maps entre visitas — reinyectarlo tira el warning de "incluiste la API de Maps más de una vez". `mount()` es `async`; usa un token de "generación" (un número que se incrementa en cada `mount()`/`unmount()`) para que, si el usuario navega a otra pestaña mientras todavía se está esperando a que cargue Maps (la primera vez que se visita en la sesión), la continuación async se dé cuenta de que ya no es la vista activa y no toque el DOM.
- **`public/admin-shell.html`** — el `<header>`/`<nav class="tabs">` escrito una sola vez (antes duplicado en 7 archivos), un `<div id="view-root">` liso, y los `<script>`: cliente de socket.io + `geo.js` + `moneyCounter.js` (clásicos, ya seguros — no declaran nada a nivel de módulo que choque), `store.js` + `router.js` + las 7 vistas + `settings.js` (`type="module"`), `theme.js` + `logout.js` (clásicos), y un bootstrap final que llama `Router.start()`.
- **`server.js`**: `app.get(PROTECTED_PAGES, (req, res) => res.sendFile(ADMIN_SHELL_PATH))`, registrado después del gate de auth y antes de `express.static` — así las 7 URLs siguen existiendo pero todas sirven el mismo shell, sin tener que mantener 7 archivos físicos idénticos a mano.

Convención en el resto de este documento: donde diga "`pedidos.js`", "`caja.js`", etc. como quien emite/escucha un evento de socket, léase `public/js/views/pedidos.js`, `public/js/views/caja.js` — la lógica no cambió de lugar conceptualmente, solo de archivo físico.
...
- **9 páginas/vistas** (`login.html`/`driver.html`/`pedido-cliente.html` son documentos HTML propios de siempre; las 6 de admin son vistas de la SPA, `public/js/views/*.js` — ver arriba. Todas comparten `style.css` y una barra `<nav class="tabs">`, más un botón "Cerrar sesión" en las de admin):
...
El botón ✏️ en `pedidos.html` abre un popup (mismo patrón `.modal-overlay`/`.modal-box` que "⚙ Configuración", ver `settings.js`) precargado con los valores actuales — Celular, Nombre, Nº de pedido, Monto, y un input por cada campo personalizado (`formConfig.customFields`). La Ubicación es la excepción: se deja **vacía** a propósito (no se re-tipea la dirección completa cada vez que se edita otra cosa); solo si el admin escribe algo ahí se resuelve con `Geo.resolveInput` (igual que al crear un pedido) y se pisan `lat`/`lng`/`label` — si se deja vacío, la ubicación existente no cambia. Al guardar, todo viaja en un solo `order:edit` con los campos que cambiaron; el handler de `server.js` ahora acepta cualquiera de `orderNumber`/`phone`/`name`/`amount`/`label`/`lat`+`lng`/`custom` (antes solo `status`/`paymentMethod`, que los dropdowns sueltos de la tabla siguen usando igual). Como cualquier cambio se sincroniza por el mismo `order:update` de siempre, el delivery lo ve reflejado al toque en su lista y en el popup del mapa — no hace falta ningún aviso aparte.
  3. **Deliverys y mapa** (`public/js/views/dashboard.js`) — lista de conectados + el mapa en vivo (los pedidos "Entregado" no muestran pin en el mapa, pero siguen en la tabla de Pedidos). El popup de cada pedido (`buildOrderPopupContent()`) tiene su propio selector **"Asignar a"** — igual que en Pedidos, pero acá el admin ve al mismo tiempo dónde queda el pedido en el mapa y a quién asignarlo, sin cambiar de pestaña. Es un elemento DOM real (no un string, como el resto de los popups) para poder colgarle el listener de `change`; asignar desde acá llama al mismo `recomputeRouteForDriver` compartido (`public/js/route-helper.js`) que usa Pedidos.
  4. **Rendición de caja** (`public/js/views/caja.js`) — ver 3.1.
  5. **Analíticas** (`public/js/views/analiticas.js`) — día comercial + histórico, ver 3.2.
  ...
  7. **Catálogo** (`public/js/views/catalogo.js`) — panel de admin para armar categorías y productos del pedido online, ver 3.3.
  8. **`pedido-cliente.html` + `pedido-cliente.js`** — la pantalla pública donde el cliente final arma su pedido, ver 3.3.
  9. **Proveedores** (`public/js/views/proveedores.js`) — panel de admin para registrar pagos a proveedores, ver 3.4.
