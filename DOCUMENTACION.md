...
- **Login de admin** (`admin_users` en Supabase, un solo usuario): `POST /api/login` verifica usuario/contraseña (`bcryptjs`) y devuelve una cookie httpOnly firmada con JWT (`SESSION_SECRET`, 30 días). `POST /api/logout` la borra. `POST /api/change-password` permite cambiarla estando logueado. Si la tabla `admin_users` está vacía al arrancar, `bootstrapAdmin()` crea el usuario inicial con `ADMIN_USERNAME`/`ADMIN_PASSWORD` (variables de entorno, solo se usan esa primera vez).
  - **Gate de páginas**: un middleware de Express redirige a `/login.html` si falta la cookie válida al pedir `nuevo-pedido.html`, `pedidos.html`, `dashboard.html`, `caja.html`, `analiticas.html`, `catalogo.html` o `proveedores.html`. Esas 7 rutas, si la cookie es válida, caen en `app.get(PROTECTED_PAGES, ...)` (justo antes de `express.static`) que sirve siempre `public/admin-shell.html` — ver "Las 7 páginas de admin son una SPA" más abajo. `driver.html`, `login.html` y `pedido-cliente.html` quedan públicas y siguen siendo archivos HTML propios de siempre — cualquier archivo en `public/` que no esté en `PROTECTED_PAGES` se sirve tal cual, no hace falta nada especial para publicar una página nueva.
...
### Las 7 páginas de admin son una SPA

`nuevo-pedido.html`, `pedidos.html`, `dashboard.html`, `caja.html`, `analiticas.html`, `catalogo.html` y `proveedores.html` dejaron de ser 7 documentos HTML independientes — las 7 URLs siguen existiendo tal cual (bookmarks, recarga dura, el gate de auth de arriba no cambia), pero ahora todas sirven el mismo `public/admin-shell.html`, y cambiar de pestaña es una navegación del lado del cliente (sin recargar el documento, sin abrir una conexión de socket nueva). `login.html`, `driver.html` y `pedido-cliente.html` **no** son parte de esto — siguen siendo páginas MPA de siempre, audiencias distintas (público general/delivery) que no se benefician de compartir un socket con el panel de admin.
...
El botón ✏️ en `pedidos.html` abre un popup (mismo patrón `.modal-overlay`/`.modal-box` que "⚙ Configuración", ver `settings.js`) precargado con los valores actuales — Celular, Nombre, Nº de pedido, Monto, y un input por cada campo personalizado (`formConfig.customFields`). La Ubicación es la excepción: se deja **vacía** a propósito (no se re-tipea la dirección completa cada vez que se edita otra cosa); solo si el admin escribe algo ahí se resuelve con `Geo.resolveInput` (igual que al crear un pedido) y se pisan `lat`/`lng`/`label` — si se deja vacío, la ubicación existente no cambia. Al guardar, todo viaja en un solo `order:edit` con los campos que cambiaron; el handler de `server.js` ahora acepta cualquiera de `orderNumber`/`phone`/`name`/`amount`/`label`/`lat`+`lng`/`custom` (antes solo `status`/`paymentMethod`, que los dropdowns sueltos de la tabla siguen usando igual). Como cualquier cambio se sincroniza por el mismo `order:update` de siempre, el delivery lo ve reflejado al toque en su lista y en el popup del mapa — no hace falta ningún aviso aparte.
  4. **Deliverys y mapa** (`public/js/views/dashboard.js`) — lista de conectados + el mapa en vivo (los pedidos "Entregado" no muestran pin en el mapa, pero siguen en la tabla de Pedidos). El popup de cada pedido (`buildOrderPopupContent()`) tiene su propio selector **"Asignar a"** — igual que en Pedidos, pero acá el admin ve al mismo tiempo dónde queda el pedido en el mapa y a quién asignarlo, sin cambiar de pestaña. Es un elemento DOM real (no un string, como el resto de los popups) para poder colgarle el listener de `change`; asignar desde acá llama al mismo `recomputeRouteForDriver` compartido (`public/js/route-helper.js`) que usa Pedidos.
  5. **Rendición de caja** (`public/js/views/caja.js`) — ver 3.1.
  6. **Analíticas** (`public/js/views/analiticas.js`) — día comercial + histórico, ver 3.2.
  ...
  8. **Catálogo** (`public/js/views/catalogo.js`) — panel de admin para armar categorías y productos del pedido online, ver 3.3.
  9. **`pedido-cliente.html` + `pedido-cliente.js`** — la pantalla pública donde el cliente final arma su pedido, ver 3.3.
  10. **Proveedores** (`public/js/views/proveedores.js`) — panel de admin para registrar pagos a proveedores, ver 3.4.
...
**Celular/Nombre/Nº de pedido/Monto volvieron a ser personalizables** (`formConfig.phone/name/orderNumber/amount: {visible, required}`) — en una primera vuelta se habían hardcodeado como fijos (para que "no dependieran de nada"), pero eso resultó ser **peor**: una configuración vieja guardada de antes (con varios en `visible:false`) quedó sin ningún toggle para revertirla, dejando esos campos invisibles para siempre tanto en `nuevo-pedido.html` como en las columnas de `pedidos.html` y en la carga masiva — un bug real que hubo que arreglar limpiando `form_config` a mano en Supabase. La lección: mejor dejarlos configurables de verdad (con su UI en Ajustes) que "fijos sin UI", porque lo segundo es una config que nadie puede corregir si queda mal. `visibleFieldOrder()` (carga masiva, `nuevo-pedido.js`) y `visibleFieldColumns()` (tabla, `pedidos.js`) vuelven a filtrar por estas 4 claves — así la carga masiva **siempre coincide exactamente** con lo que pide el formulario individual.
