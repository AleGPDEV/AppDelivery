...
`style.css` es compartido por las 10 páginas, con un solo set de tokens (`--primary`, `--primary-dark`, `--primary-glow`, `--primary-soft`) que cambia según el **rol** de la página, no por página individual — así las 7 pantallas de admin (uso interno, la misma persona salta entre ellas) se sienten un solo tema, mientras el cliente final y el delivery (audiencias distintas, en su propio celular) tienen su propio color de identidad. El mecanismo es la misma clase `<body class="role-*">` leída por CSS, sin nada en JS:
