...
- **`caja.js`**: el input "Efectivo inicial" de cada caja de delivery emite `driver:cash-start` en cada tecla. Importante: los elementos de cada caja (inputs, textos) se crean **una sola vez** por delivery (`ensureBox()`/`driverBoxes` Map) y se actualizan en su lugar (`updateBox()`) — la versión anterior reconstruía toda la lista en cada cambio, lo que le hacía perder el foco al input en cada tecla tipeada (mismo tipo de bug que el de los `<select>` que se cerraban solos, ver más abajo). `updateBox()` tampoco pisa el valor de un input mientras tiene el foco (evita que el eco del propio cambio recién tipeado lo interrumpa).
...
### Borrar pedidos activos ("Zona de pruebas" en analiticas.html)

`POST /api/admin/reset-today` — borra todos los pedidos **activos** (`!archivedAt`, de Supabase y en memoria, con su `order:remove` de broadcast) y, si hay un día abierto, lo **cancela sin cerrarlo de verdad** (lo borra de `business_days` en vez de pasar por `/end`, así no pide efectivo ni queda nada en el historial). También limpia `driverCashStarts` (emitiendo `driver:cash-start` con `amount: 0` a cada uno antes). Pensado para poder probar la app sin ensuciar los datos reales — **no toca** pedidos ya archivados ni días ya cerrados, eso es historial real. El botón en `analiticas.js` arma el `confirm()` con la cantidad real de pedidos activos (mismo `orders` Map que ya usa para el aviso de "Finalizar día").
