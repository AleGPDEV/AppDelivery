...
- Ninguno de estos 4 endpoints es un evento de Socket.IO — son HTTP normales protegidos por el middleware `requireAuth` (mismo criterio que `socket.data.isAdmin`, respeta `DISABLE_AUTH`). El evento `business-day:status` sí es un socket (broadcast de solo lectura, sin gate de admin — no hay nada sensible en saber si hay un día abierto).
