...
- **Login de admin** (`admin_users` en Supabase, un solo usuario): `POST /api/login` verifica usuario/contraseña (`bcryptjs`) y devuelve una cookie httpOnly firmada con JWT (`SESSION_SECRET`, 30 días). `POST /api/logout` la borra. `POST /api/change-password` permite cambiarla estando logueado. Si la tabla `admin_users` está vacía al arrancar, `bootstrapAdmin()` crea el usuario inicial con `ADMIN_USERNAME`/`ADMIN_PASSWORD` (variables de entorno, solo se usan esa primera vez).
  - **Gate de páginas**: un middleware de Express redirige a `/login.html` si falta la cookie válida al pedir `pedidos.html`, `dashboard.html`, `analiticas.html`, `catalogo.html` o `proveedores.html`. Esas 5 rutas, si la cookie es válida, caen en `app.get(PROTECTED_PAGES, ...)` (justo antes de `express.static`) que sirve siempre `public/admin-shell.html` — ver "Las páginas de admin son una SPA" más abajo. `driver.html`, `login.html` y `pedido-cliente.html` quedan públicas y siguen siendo archivos HTML propios de siempre — cualquier archivo en `public/` que no esté en `PROTECTED_PAGES` se sirve tal cual, no hace falta nada especial para publicar una página nueva.
...
- `public/settings.js` — el botón **"⚙ Configuración"** del shell de admin (ver 3.4 para el detalle de qué configura). Se auto-monta al cargar: crea el botón e inyecta el modal entero en `document.body`. `<script type="module">` que importa `Store` directamente (`Store.socket`, `Store.getFormConfig()`) — antes abría su propia conexión de socket independiente (dos sockets por página, uno de la página y otro de acá); ya no hace falta, y tampoco necesita el IIFE manual que usaba para no chocar con los `const socket`/`let formConfig` de cada página clásica (los módulos ES tienen su propio scope real, ese problema ya no existe).
  - **Pestañas adentro del modal** (`.settings-tabs`, `[data-settings-tab]`/`[data-settings-panel]`): el contenido se agrupa en 3 secciones — **Pedidos** (Reglas fijas, Personalizar campos del formulario, Campos personalizados), **Delivery** (Métodos de pago — son los mismos botones que usa el delivery al marcar un pedido entregado), **Cuenta** (cambio de contraseña). El click en una pestaña solo togglea `hidden` en el panel correspondiente, sin re-renderizar nada — el contenido de cada panel ya está armado de antes por `renderBuiltinFieldConfig()`/`renderPaymentMethods()`/`renderFieldConfig()`, que siguen corriendo igual que antes (una sola vez al recibir `form-config:snapshot`, no por pestaña).

### Las páginas de admin son una SPA

`pedidos.html`, `dashboard.html`, `analiticas.html`, `catalogo.html` y `proveedores.html` dejaron de ser documentos HTML independientes — las 5 URLs siguen existiendo tal cual (bookmarks, recarga dura, el gate de auth de arriba no cambia), pero ahora todas sirven el mismo `public/admin-shell.html`, y cambiar de pestaña es una navegación del lado del cliente (sin recargar el documento, sin abrir una conexión de socket nueva). `login.html`, `driver.html` y `pedido-cliente.html` **no** son parte de esto — siguen siendo páginas MPA de siempre, audiencias distintas (público general/delivery) que no se benefician de compartir un socket con el panel de admin. (Hubo dos vistas más que ya no existen como pestañas propias: "Nuevo pedido", que ahora es un modal dentro de Pedidos, y "Rendición de caja", cuya lógica se fusionó dentro de Dashboard — ver 3.1.)
...
- **`public/js/views/*.js`** (`nuevo-pedido.js`, `pedidos.js`, `dashboard.js`, `analiticas.js`, `catalogo.js`, `proveedores.js`) — un `<script type="module">` por vista (scope real por archivo, sin colisiones aunque coexistan en el mismo documento — antes, como `<script>` clásicos, varias declaraban `const socket`/`let formConfig` a nivel de módulo, que hoy chocarían si convivieran). Cada uno registra su vista con `Router.register(path, {...})` (excepto `nuevo-pedido.js`, que ya no es una vista propia — ver más abajo). Todo `document.getElementById`/`addEventListener` que antes vivía a nivel de módulo (funcionaba porque el `<script>` estaba al final del `<body>`, después de que el HTML ya existía) se movió adentro de `mount(root)`, consultando `root.querySelector(...)` — la vista se monta recién cuando su HTML ya está en el DOM. `unmount()` reemplaza el contenedor, así que los listeners atados a esos nodos se recolectan solos; solo hace falta limpiar a mano lo que vive *fuera* del DOM: suscripciones a `Store`, timers (`analiticas.js` tiene un debounce de 600ms para el efectivo inicial), y en `dashboard.js` el `setInterval` de 5s + los markers/InfoWindows de Google Maps (`marker.setMap(null)`).
  ...
  - **`dashboard.js`** es la vista más particular: `loadGoogleMaps()`/su promesa de caché quedan a nivel de módulo (no adentro de `mount()`) para reusar la carga del script de Maps entre visitas — reinyectarlo tira el warning de "incluiste la API de Maps más de una vez". `mount()` es `async`; usa un token de "generación" (un número que se incrementa en cada `mount()`/`unmount()`) para que, si el usuario navega a otra pestaña mientras todavía se está esperando a que cargue Maps (la primera vez que se visita en la sesión), la continuación async se dé cuenta de que ya no es la vista activa y no toque el DOM. Además del mapa, esta vista arma una tarjeta de rendición por cada delivery (`#driver-cards`, ver 3.1) — dejó de existir como pestaña propia ("Rendición de caja"/`caja.js`) y ahora vive al lado del mapa, en la misma pantalla.
...
- **8 páginas/vistas** (`login.html`/`driver.html`/`pedido-cliente.html` son documentos HTML propios de siempre; las 5 de admin son vistas de la SPA, `public/js/views/*.js` — ver arriba. Todas comparten `style.css` y una barra `<nav class="tabs">`, más un botón "Cerrar sesión" en las de admin):
...
El botón ✏️ en `pedidos.html` abre un popup (mismo patrón `.modal-overlay`/`.modal-box` que "⚙ Configuración", ver `settings.js`) precargado con los valores actuales — Celular, Nombre, Nº de pedido, Monto, y un input por cada campo personalizado (`formConfig.customFields`). La Ubicación es la excepción: se deja **vacía** a propósito (no se re-tipea la dirección completa cada vez que se edita otra cosa); solo si el admin escribe algo ahí se resuelve con `Geo.resolveInput` (igual que al crear un pedido) y se pisan `lat`/`lng`/`label` — si se deja vacío, la ubicación existente no cambia. Al guardar, todo viaja en un solo `order:edit` con los campos que cambiaron; el handler de `server.js` ahora acepta cualquiera de `orderNumber`/`phone`/`name`/`amount`/`label`/`lat`+`lng`/`custom` (antes solo `status`/`paymentMethod`, que los dropdowns sueltos de la tabla siguen usando igual). Como cualquier cambio se sincroniza por el mismo `order:update` de siempre, el delivery lo ve reflejado al toque en su lista y en el popup del mapa — no hace falta ningún aviso aparte.
  ...
  4. **Analíticas** (`public/js/views/analiticas.js`) — día comercial + histórico, ver 3.2.
  ...
  8. **Proveedores** (`public/js/views/proveedores.js`) — panel de admin para registrar pagos a proveedores, ver 3.4.
...
- **Qué delivery tiene tarjeta**: la unión de todo `assignedTo` no nulo entre los pedidos y todos los deliverys actualmente conectados (`renderDriverCards()`) — así un delivery que se desconectó pero todavía tiene pedidos sin rendir sigue teniendo tarjeta, y uno recién conectado sin nada asignado también aparece (para poder cargarle el Cambio inicial antes de asignarle el primer pedido).
...
- **`#products-view`** (al tocar una tarjeta): header con flecha "←" (`#back-to-categories-btn`, vuelve a la grilla) + el nombre de la categoría, y la grilla de productos de siempre (`.product-card`, foto/nombre/descripción/precio/stepper +/-, sin cambios ahí — solo se filtra a `activeCategoryId` en vez de mostrar las 10 categorías apiladas).
...
**El botón "⚙ Configuración" (`settings.js`) aparece en las 6 páginas de admin** — ver la descripción de `public/settings.js` más arriba. Adentro tiene: **Reglas fijas** (texto informativo, sin checkboxes — ver más abajo qué es lo único realmente fijo), **Personalizar campos del formulario** (Celular/Nombre/Nº de pedido/Monto — mostrar/ocultar y marcar obligatorio, igual que un campo personalizado), **Métodos de pago**, **Campos personalizados** (agregar campos propios, además de los 4 de arriba) y **Cuenta** (cambiar contraseña).

**Lo único realmente fijo es el tipo de envío** (retira/envía) — nunca fue un campo de `formConfig`, siempre es obligatorio, no tiene toggle:
...
- Tanto `order:add` (admin) como `order:web-add` (cliente) **validan esto del lado del servidor**: rechazan si no es retiro y no hay `lat`/`lng` resueltos — antes esa validación en `order:web-add` estaba rota (comparaba contra un campo `payload.location` que el cliente nunca manda, así que en la práctica nunca bloqueaba nada).

**Celular/Nombre/Nº de pedido/Monto volvieron a ser personalizables** (`formConfig.phone/name/orderNumber/amount: {visible, required}`) — en una primera vuelta se habían hardcodeado como fijos (para que "no dependieran de nada"), pero eso resultó ser **peor**: una configuración vieja guardada de antes (con varios en `visible:false`) quedó sin ningún toggle para revertirla, dejando esos campos invisibles para siempre tanto en el formulario de "Nuevo pedido" como en las columnas de `pedidos.html` y en la carga masiva — un bug real que hubo que arreglar limpiando `form_config` a mano en Supabase. La lección: mejor dejarlos configurables de verdad (con su UI en Ajustes) que "fijos sin UI", porque lo segundo es una config que nadie puede corregir si queda mal. `visibleFieldOrder()` (carga masiva, `nuevo-pedido.js`) y `visibleFieldColumns()` (tabla, `pedidos.js`) vuelven a filtrar por estas 4 claves — así la carga masiva **siempre coincide exactamente** con lo que pide el formulario individual.
