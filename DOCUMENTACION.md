...
- **BUG confirmado: la asignación de un pedido a un delivery nunca se guarda en Supabase.** `orders.assigned_to` tiene una foreign key (`orders_assigned_to_fkey`) contra una tabla `drivers` que existe pero está siempre vacía — los deliverys son "en memoria solamente" a propósito (ver arriba), nunca se insertan ahí. Resultado: cada `order:assign` (desde Pedidos, Nuevo pedido, o el popup del mapa) actualiza la memoria y se ve bien en la sesión actual, pero `persistOrder` falla en silencio (`Error guardando pedido en Supabase: ... violates foreign key constraint "orders_assigned_to_fkey"`, solo visible en los logs de Render). Un reinicio del servidor (Render duerme el free tier tras 15 min sin tráfico) borra todas las asignaciones sin aviso. Confirmado en producción: ningún pedido de los que hay hoy tiene `assigned_to` guardado. **Pendiente de arreglar** — la solución es sacar esa foreign key (ya que no hay tabla de deliverys real) o insertar/actualizar una fila en `drivers` cada vez que uno se conecta.
